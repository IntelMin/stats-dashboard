          ...
          <Accordion key={index} className={b('row')} expanded={expanded === `panel${index}`} onChange={handleChange(`panel${index}`)}>
            ...
            <AccordionDetails>
              <Grid className="sub-row" container spacing={10}>
                ...
                <Grid item xs={12} md={6}>
                  <div className="flex"><span>Total Longs:</span><span>{(BNtoNum(market.totalLongs))}</span></div>
                  <div className="flex"><span>Total Shorts:</span><span>{(BNtoNum(market.totalShorts))}</span></div>
                  <div className="flex"><span>Total Longs Count (Historical): </span><span>{(market.totalLongsHistorical)}</span></div>
                  <div className="flex"><span>Total Shorts Count (Historical):</span><span>{(market.totalShortsHistorical)}</span></div>
